...
transformations. They are in particular important for deforming geometries
as linear blending is a very close approximation of closest path blending,
which is not the case for any other representation.
...
def conjugate(dual_quaternion, name=None):
  ...
  with tf.compat.v1.name_scope(name, "dual_quaternion_conjugate",
    ...
    quaternion_real = quaternion.conjugate(quaternion_real)
    quaternion_dual = quaternion.conjugate(quaternion_dual)
