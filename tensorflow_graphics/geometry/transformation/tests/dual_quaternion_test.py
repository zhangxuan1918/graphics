...
class DualQuaternionTest(test_case.TestCase):
  ...
  def test_conjugate_consistency(self):
    """Test if the conjugate function is consistent with quaternion module."""
    x_init = test_helpers.generate_preset_test_dual_quaternions()
    x = tf.convert_to_tensor(value=x_init)
    y = tf.convert_to_tensor(value=x_init)

    x = dual_quaternion.conjugate(x)
    x_real, x_dual = tf.split(y, (4, 4), axis=-1)

    y_real, y_dual = tf.split(y, (4, 4), axis=-1)
    y_real = quaternion.conjugate(y_real)
    y_dual = quaternion.conjugate(y_dual)

    self.assertAllEqual(x_real, y_real)
    self.assertAllEqual(x_dual, y_dual)

  def test_conjugate_correctness(self):
    """Test if the conjugate function is providing correct results."""
    x_init = test_helpers.generate_preset_test_dual_quaternions()
    x = tf.convert_to_tensor(value=x_init)
    y = tf.convert_to_tensor(value=x_init)
